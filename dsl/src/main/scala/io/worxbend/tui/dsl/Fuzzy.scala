...
private[dsl] object Fuzzy:
  ...
  /** A predicate accepting the candidates that match `query` (an empty query accepts everything). The query is folded
    ...
    *
    * Case folding uses [[Locale.ROOT]], not the default locale: under a Turkish or Azeri locale `"Install"` folds to
    * `"ınstall"`, so a query containing `i` would match nothing and the palette would appear empty.
    */
  def matcher(query: String): String => Boolean =
    ...
    candidate =>
      ...
      candidate
        ...
        .foreach(c => if matched < needle.length && needle.charAt(matched) == c then matched += 1)
