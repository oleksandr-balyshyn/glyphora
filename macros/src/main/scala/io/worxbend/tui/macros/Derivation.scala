...
/** Handles an application action value. Bound at compile time via [[bindAction]] — never through runtime reflection
  * (the TamboUI `@OnAction` lesson, SPEC.md §6).
  */
...
/** Binds a handler function to an action type at compile time. The `inline` keeps the dispatch a direct call in the
  * generated code — no lookup table, no reflective invocation.
  */
