...
/** One rendered terminal cell.
  ...
  * characters, emoji ZWJ sequences). A wide (two-column) grapheme lives in its left cell; the cell to its right
  * is a continuation filler that backends skip when flushing.
  */
