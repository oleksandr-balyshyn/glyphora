...
object KeyEvent:
  ...
  def parse(spec: String): Either[String, KeyEvent] =
    ...
    if spec.isEmpty then Left("empty spec")
      ...
      val (rawKeyName, modifiers) = stripModifiers(dropLeadingPadding(spec), KeyModifiers.None)
      val keyName                 = dropSurroundingPadding(rawKeyName)
  ...
  /** Strips a spec's leading whitespace padding — but never all of it, because a value that is *only* whitespace is the
    * space key rather than padding. Leading padding comes off before the modifier scan so `"ctrl+ "` keeps its trailing
    * space for the key name.
    */
  private def dropLeadingPadding(text: String): String =
    if text.isBlank then text else text.dropWhile(_.isWhitespace)

  /** Trims a key name's padding on both sides, with the same caveat [[dropLeadingPadding]] has: a name that is *only*
    * whitespace is the space key rather than padding.
    */
  private def dropSurroundingPadding(text: String): String =
    if text.isBlank then text else text.trim
  ...
  private def keyCodeFor(name: String): Either[String, KeyCode] =
    ...
    NamedKeys.get(lowered) match
      case Some(code) => Right(code)
      case None       =>
        if lowered.startsWith("f") && lowered.drop(1).toIntOption.exists(n => n >= 1 && n <= MaxFunctionKey) then
          Right(KeyCode.F(lowered.drop(1).toInt))
        ...
        else if name.codePointCount(0, name.length) == 1 then Right(KeyCode.Char(name.codePointAt(0)))
        else Left(s"unknown key '$name'")

  /** The named keys a spec can spell, keyed by lowercase name; aliases (`esc`/`escape`, `del`/`delete`, `pgup`,
    * `prtsc`, the media-key spellings, …) are duplicate entries pointing at the same code.
    *
    * `"pause"` is the Pause/Break key, so the media one needs its own spelling: `"mediapause"`. `"playpause"` is the
    * single toggle button most keyboards actually have. The single-codepoint fallback and the `f`-key range are not
    * here — they are guards in [[keyCodeFor]] after this lookup.
    */
  private val NamedKeys: Map[String, KeyCode] = Map(
    "enter"         -> KeyCode.Enter,
    "esc"           -> KeyCode.Escape,
    ...
    "pause"         -> KeyCode.Pause,
    "menu"          -> KeyCode.Menu,
    "play"          -> KeyCode.Media(MediaKey.Play),
    "mediapause"    -> KeyCode.Media(MediaKey.Pause),
    "playpause"     -> KeyCode.Media(MediaKey.PlayPause),
    "reverse"       -> KeyCode.Media(MediaKey.Reverse),
    "stop"          -> KeyCode.Media(MediaKey.Stop),
    "fastforward"   -> KeyCode.Media(MediaKey.FastForward),
    "rewind"        -> KeyCode.Media(MediaKey.Rewind),
    "next"          -> KeyCode.Media(MediaKey.TrackNext),
    "tracknext"     -> KeyCode.Media(MediaKey.TrackNext),
    "prev"          -> KeyCode.Media(MediaKey.TrackPrevious),
    "trackprev"     -> KeyCode.Media(MediaKey.TrackPrevious),
    "trackprevious" -> KeyCode.Media(MediaKey.TrackPrevious),
    "record"        -> KeyCode.Media(MediaKey.Record),
    "volumedown"    -> KeyCode.Media(MediaKey.LowerVolume),
    "voldown"       -> KeyCode.Media(MediaKey.LowerVolume),
    "volumeup"      -> KeyCode.Media(MediaKey.RaiseVolume),
    "volup"         -> KeyCode.Media(MediaKey.RaiseVolume),
    "mute"          -> KeyCode.Media(MediaKey.MuteVolume),
  )
