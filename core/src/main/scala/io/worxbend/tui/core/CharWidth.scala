...
/** Terminal display-width arithmetic.
  *
  * `String.length` counts UTF-16 code units, not terminal columns: CJK characters occupy two columns, combining
  * marks occupy none, and one emoji ZWJ sequence can span many codepoints yet occupy two columns. All layout and
  * rendering math in this library must go through this object — no code outside it may use `String.length` or
  * `String.substring` for anything that affects layout (SPEC.md §2.4).
  *
  * Widths are derived from the Unicode Character Database: East Asian Width `W`/`F` codepoints (see
  * [[WidthTable]]) are two columns; combining marks, format controls, and conjoining Hangul jamo are zero;
  * everything else is one. Variation selectors override the base width (VS16 forces emoji presentation, two
  * columns; VS15 forces text presentation, one column), and a regional-indicator pair (flag emoji) is two.
  */
object CharWidth:
  ...
  /** Medial vowels and trailing consonants of decomposed Hangul syllables render inside the leading consonant's
    * two columns rather than adding their own.
    */
