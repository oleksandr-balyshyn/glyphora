...
final class ProgressPresetSpec extends AnyFunSuite:
  ...
  test("a sub-cell style resolves finer than one cell, a whole-cell style does not"):
    def distinctRenderings(style: ProgressPreset): Int =
      (0 to 800).map(step => style.glyphs(step / 800.0, 8)).distinct.size
    assert(!ProgressPreset.Line.isSubCell)
    assert(ProgressPreset.Blocks.isSubCell)
    assert(distinctRenderings(ProgressPreset.Line) == 9, "a whole-cell bar has one rendering per cell boundary")
    assert(distinctRenderings(ProgressPreset.Blocks) > 50, "eight partials should multiply the resolution")
  ...
  test("sub-cell styles floor the fill and whole-cell styles round it"):
    assert(ProgressPreset.Line.filledCells(0.6, 10) == 6)
    assert(ProgressPreset.Line.filledCells(0.65, 10) == 7, "rounds up to the nearest cell")
    assert(ProgressPreset.Blocks.filledCells(0.65, 10) == 6, "floors, and the remainder becomes a partial glyph")
    assert(ProgressPreset.Blocks.glyphs(0.65, 10)(6) != ProgressPreset.Blocks.track, "the boundary cell is drawn")
  ...
  test("a head glyph caps the fill while the bar is incomplete"):
    val arrow = ProgressPreset.Arrow
  ...
  test("a line gauge draws with whichever style it is given"):
    assert(trimmedLines(rendered(LineGauge(0.5, preset = ProgressPreset.Ascii), 12, 1)) == Seq("50% ####----"))
    assert(trimmedLines(rendered(LineGauge(0.5, preset = ProgressPreset.Arrow), 12, 1)) == Seq("50% ===>----"))
