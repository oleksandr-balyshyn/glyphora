...
final class OrbitSpinnerSpec extends AnyFunSuite:
  ...
  test("the same moment renders the same frame twice"):
    OrbitPath.values.foreach: path =>
      val spinner = OrbitSpinner(700.millis, OrbitSpinnerOptions(path = path))
  ...
  test("an orbit never writes outside the rect it is given"):
    ...
    OrbitSpinner(400.millis, OrbitSpinnerOptions(radius = Some(3))).render(Rect(2, 1, 10, 6), frame)
      ...
      Seq(0, 1, 2, 3).foreach: size =>
        Seq(Some(0), Some(1), Some(99), None).foreach: radius =>
          Seq(-1.0, 0.0, 0.5, 1.0, 2.0, Double.NaN).foreach: sweep =>
            val spinner = OrbitSpinner(250.millis, OrbitSpinnerOptions(path = path, radius = radius, sweep = sweep))
  ...
  test("a zero period parks the arc instead of dividing by zero"):
    val parked = OrbitSpinner(500.millis, OrbitSpinnerOptions(period = Duration.Zero))
    ...
    (0 until 8).foreach: sample =>
      ...
      def draw(direction: SpinDirection) =
        rendered(
          OrbitSpinner(
            moment,
            OrbitSpinnerOptions(radius = Some(3), resolution = CanvasResolution.Cell, direction = direction),
          ),
          width,
          height,
        )
  ...
  test("a full solid sweep lights the whole ring and stops moving"):
    val solid = OrbitSpinner(0.millis, OrbitSpinnerOptions(sweep = 1.0, radius = Some(3), trail = OrbitTrail.Solid))
    assert(frameAt(solid) == frameAt(solid.copy(elapsed = 900.millis)), "a fully lit solid ring should be static")

    val comet = OrbitSpinner(0.millis, OrbitSpinnerOptions(sweep = 1.0, radius = Some(3), trail = OrbitTrail.Comet()))
  ...
  test("a wider sweep lights more of the ring than a narrow one"):
    def litCells(sweep: Double): Int =
      val buffer = rendered(
        OrbitSpinner(0.millis, OrbitSpinnerOptions(sweep = sweep, radius = Some(4), style = Style.Default.dim)),
        16,
        8,
      )
      (for
  ...
  test("a fixed radius paints inside the size it claims"):
    Seq(CanvasResolution.Braille, CanvasResolution.HalfBlock, CanvasResolution.Cell).foreach: resolution =>
      Seq(1, 2, 3, 5).foreach: radius =>
        val spinner = OrbitSpinner(300.millis, OrbitSpinnerOptions(radius = Some(radius), resolution = resolution))
  ...
  test("a fitted orbit claims no size, because it takes whatever it is given"):
    assert(OrbitSpinner(0.millis, OrbitSpinnerOptions(radius = None)).preferredSize.isEmpty)
  ...
  test("every resolution draws a figure, and the cell resolution uses the marker"):
    Seq(CanvasResolution.Braille, CanvasResolution.HalfBlock).foreach: resolution =>
      val drawn = frameAt(OrbitSpinner(300.millis, OrbitSpinnerOptions(resolution = resolution, radius = Some(2))))
      assert(drawn.exists(_._1.trim.nonEmpty), s"$resolution drew nothing")

    val ascii = frameAt(
      OrbitSpinner(300.millis, OrbitSpinnerOptions(resolution = CanvasResolution.Cell, marker = "*", radius = Some(2)))
    )
  ...
  test("a square path is squarer than a circle at the same radius"):
    def corners(path: OrbitPath): Int =
      val buffer =
        rendered(OrbitSpinner(0.millis, OrbitSpinnerOptions(path = path, radius = Some(3), sweep = 1.0)), 16, 8)
    ...
    assert(
      frameAt(OrbitSpinner(0.millis, OrbitSpinnerOptions(path = OrbitPath.Square, radius = Some(3), sweep = 1.0))) !=
        frameAt(OrbitSpinner(0.millis, OrbitSpinnerOptions(path = OrbitPath.Circle, radius = Some(3), sweep = 1.0)))
    )
