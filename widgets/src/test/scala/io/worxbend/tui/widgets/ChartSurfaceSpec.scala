...
final class ChartSurfaceSpec extends AnyFunSuite:
  ...
  test("y labels run bottom to top, right-aligned against the axis"):
    val chart  = Chart(Seq.empty, Bounds(0.0, 1.0), Bounds(0.0, 1.0), ChartOptions(yLabels = Seq("lo", "mid", "hi")))
  ...
  test("a single y label sits on the axis origin row"):
    val chart  = Chart(Seq.empty, Bounds(0.0, 1.0), Bounds(0.0, 1.0), ChartOptions(yLabels = Seq("0")))
  ...
  test("explicit y labels replace the two auto-formatted bounds rather than joining them"):
    val chart =
      Chart(Seq.empty, Bounds(0.0, 100.0), Bounds(0.0, 100.0), ChartOptions(showLabels = true, yLabels = Seq("a", "b")))
  ...
  test("the gutter is measured in columns, so a wide-character label still clears the plot"):
    val chart  = Chart(Seq(diagonal), Bounds(0.0, 10.0), Bounds(0.0, 10.0), ChartOptions(yLabels = Seq("你好世界")))
    val buffer = rendered(chart, 12, 5)
  ...
  test("a pane too narrow for the gutter drops the y labels and keeps the plot"):
    val chart  = Chart(Seq.empty, Bounds(0.0, 1.0), Bounds(0.0, 1.0), ChartOptions(yLabels = Seq("a very long label")))
    val buffer = rendered(chart, 6, 4)
    assert(buffer.get(0, 3).symbol == "└")

  test("a dataset marker overrides the chart marker"):
    val chart  =
      Chart(Seq(diagonal.copy(marker = Some("*"))), Bounds(0.0, 10.0), Bounds(0.0, 10.0), ChartOptions(marker = "•"))
  ...
  test("datasets keep their own resolutions in one plot"):
    ...
    val buffer  = rendered(Chart(Seq(braille, cells), Bounds(0.0, 10.0), Bounds(0.0, 10.0)), 12, 6)
  ...
  test("a dataset with no overrides follows the chart, in a single pass"):
    val chart  = Chart(Seq(diagonal), Bounds(0.0, 10.0), Bounds(0.0, 10.0), ChartOptions(marker = "#"))
    val buffer = rendered(chart, 6, 5)
    assert(buffer.get(1, 3).symbol == "#")
  ...
  test("datasets sharing a surface draw in listing order, so the later one is on top"):
    ...
    val buffer = rendered(Chart(Seq(under, over), Bounds(0.0, 10.0), Bounds(0.0, 10.0)), 7, 7)
    val fills  = for
      row    <- 0 until 6
      column <- 1 until 7
  ...
  test("a dataset with no points and an override still leaves the axes drawn"):
    ...
    val buffer = rendered(Chart(Seq(empty), Bounds(0.0, 1.0), Bounds(0.0, 1.0)), 5, 4)
