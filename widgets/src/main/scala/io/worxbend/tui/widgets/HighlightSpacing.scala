...
/** When a list reserves the gutter its highlight symbol is drawn in.
  ...
  *     width for text, and the text shifts right by the marker's width the first time a row is highlighted. In a
  *     narrow pane those two reclaimed columns are worth the shift.
  *   - [[Never]] reserves nothing and draws no marker at all. Use it when the highlight *style* — a reversed or
  *     coloured row — is the only cue wanted, which is the usual choice for a list inside a bordered panel where
  *     every column counts.
  *
