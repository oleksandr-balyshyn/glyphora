...
/** A modal-style dialog drawn over existing content: clears a centered box, borders it, renders the message
  * and a row of buttons with one highlighted.
  *
  * There is no screen stack (SPEC.md §7 non-goal) — a dialog is just an element rendered last in the view,
  * so it paints over whatever was drawn before it.
  */
