...
import io.worxbend.tui.terminal.ScriptedInput.{NothingAvailable, csi, decoder}
...
final class CursorReportSpec extends AnyFunSuite:
  ...
  test("a report is read as a zero-based position, converted from the one-based wire format"):
    ...
    val input = decoder(csi("12;40R")*)
    assert(input.readCursorReport(50.millis) == Some(Position(39, 11)))

  test("the top-left corner reports as the origin rather than as row one"):
    assert(decoder(csi("1;1R")*).readCursorReport(50.millis) == Some(Position(0, 0)))
  ...
  test("outside a query the very same bytes are still the F3 key"):
    ...
    val input = decoder(csi("1;5R")*)
    assert(input.decode(10) == Some(Event.Key(KeyEvent(KeyCode.F(3), KeyModifiers.Ctrl))))
  ...
  test("inside a query the same shape is read as the report it is"):
    ...
    assert(decoder(csi("1;5R")*).readCursorReport(50.millis) == Some(Position(4, 0)))

  test("the ambiguity closes again as soon as the query is answered"):
    val input = decoder((csi("3;3R") ++ csi("1;5R"))*)
    assert(input.readCursorReport(50.millis) == Some(Position(2, 2)))
    assert(input.decode(10) == Some(Event.Key(KeyEvent(KeyCode.F(3), KeyModifiers.Ctrl))))
  ...
  test("keys typed during the round trip are delivered afterwards, in the order they were typed"):
    ...
    val input = decoder(('a'.toInt +: 'b'.toInt +: csi("2;7R"))*)
    assert(input.readCursorReport(50.millis) == Some(Position(6, 1)))
    assert(input.decode(10) == Some(Event.Key(KeyEvent(KeyCode.Char('a'), KeyModifiers.None))))
    assert(input.decode(10) == Some(Event.Key(KeyEvent(KeyCode.Char('b'), KeyModifiers.None))))
  ...
  test("a deferred key is delivered even when the query itself timed out"):
    ...
    val input = decoder(('x'.toInt +: Seq.empty)*)
    assert(input.readCursorReport(20.millis).isEmpty)
    assert(input.decode(10) == Some(Event.Key(KeyEvent(KeyCode.Char('x'), KeyModifiers.None))))
  ...
  test("a malformed report is not mistaken for a position"):
    ...
    assert(decoder(csi("5R")*).readCursorReport(20.millis).isEmpty)
    assert(decoder(csi("0;4R")*).readCursorReport(20.millis).isEmpty)
