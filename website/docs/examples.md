...
Runnable example apps in [`examples/`](https://github.com/oleksandr-balyshyn/glyphora/tree/main/examples)
are the primary "how do I use this" documentation and the GraalVM native-image
compile targets. Each has headless `Pilot` end-to-end tests in its `test` submodule.
...
`--no-fallback`, no reflect-config needed) — see [Native binaries](./native-image).
